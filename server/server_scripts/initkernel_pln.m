...
socketEventHandler[data_] := Block[
	...
	ClientLibrary`debug["Done evaluating."];
        (* I believe ExportString is a way to convert to JSON *)
        (* WARNING: This call to ToString would seem to be superfluous,
         but it is not *)
        exprString = ToString[ExportString[expr,"ExpressionJSON"]]
        (* uncommenting the next line breaks our system!*)
        (* Print[Expr] *)
        (*        WriteString[
		$OutputSocket,
                exprString
